...
- **Existing PBTs in another framework** (proptest, quickcheck, etc.) should be
  ...
  language-specific porting reference (e.g., `references/porting-rust.md` or
  `references/porting-php.md`).
...
`test_hegel.rs`, `PropertyTest.php`, or similar — property-based tests are
...
1. **Add tests to the appropriate existing test file.** If there's already a
   `test_foo.rs` or `FooTest.php` covering the module, add hegel tests there.
   Only create a new file if no relevant test file exists.
...
many real libraries. Most of the current examples were first validated against
Rust crate libraries, but the same property shapes transfer directly to PHP and
other Hegel SDKs. See `references/field-tested-patterns.md` for detailed
examples.
...
The examples in this section use Rust syntax for brevity. When working in PHP,
apply the same discipline with `Hegel\Generators`; see `references/php.md` for
...
Instead, use hegel's language-specific random support so the random decisions
stay shrinkable. See the language-specific reference for API details.

For PHP specifically, do **not** use `mt_srand()`, a seeded `Random\Randomizer`,
or a hegel-generated integer seed as a stand-in for shrinkable randomness. Use
`$tc->randomizer()` by default, or `$tc->randomizer(true)` only when the code
under test genuinely requires true random-looking output.
...
Requires [`uv`](https://github.com/astral-sh/uv) on PATH and Pest in the test
suite. Run with `./vendor/bin/pest`.

```php
use Hegel\Generators;
use Hegel\TestCase;

hegel('reverse is an involution', function (TestCase $tc) {
